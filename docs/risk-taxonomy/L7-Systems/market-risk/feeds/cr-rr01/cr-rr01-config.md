---
# Document Metadata
document_id: CR-RR01-CFG-001
document_name: CR RR01 Feed - IT Configuration Specification
version: 1.0
...
# CR RR01 Feed - IT Configuration Specification

**Meridian Global Bank - Risk Technology**

| Document Control | |
|-----------------|---|
| **Document ID** | CR-RR01-CFG-001 |
| **Version** | 1.0 |
...
This document provides the technical configuration details for the CR RR01 (Credit Recovery Rate 01) feed within the Murex Global Object Model (GOM). This feed extracts recovery rate sensitivity WITH propagation for credit products.
...
|----------|-------|
| **Feed Name** | CR RR01 |
| **File Pattern** | MxMGB_MR_Credit_RR01_{Region}_{YYYYMMDD}.csv |
| **Source Tables** | TBL_VESPA_SENS_REP (Non-CRDI) + TBL_VESPA_SENSCI_REP (CRDI) |
| **Structure** | UNION ALL |
| **Key Metric** | M_RECOVERY_ (RR01_SENSI) |
| **Output Fields** | 20 |
| **Delimiter** | Semicolon (;) |
| **Encoding** | UTF-8 |

### 1.2 Comparison with CR Basis Rate / CR RR02
...
| CR Basis Rate | M_RECOVERY1 | WITHOUT | Same as RR02 |
...
┌─────────────────────────────────────────────────────────────────┐
│                     MUREX FRONT OFFICE                          │
│           Valuation with Recovery Rate Bumping                  │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────────┐
│                  REGIONAL PROCESSING                            │
│  LN_MR_VESPA_R1_RPT | HK_MR_VESPA_R1_RPT |                      │
│  NY_MR_VESPA_R1_RPT | SP_MR_VESPA_R1_RPT                        │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
| **Key Fields** | M_TRADE_NUM, M_RECOVERY_, M_RATE, M_DATE__ZER |
...
| **Key Fields** | M_TRADE_NUM, M_RECOVERY_, M_PL_INSTRU |
| **Filter** | M_GROUP = 'CRDI' |
...
| Property | Value |
|----------|-------|
| **Extractor Name** | DE_VESPA_CR_RR01 |
...
### 4.5 Key Differences from CR Basis Rate

| Aspect | CR RR01 | CR Basis Rate |
|--------|---------|---------------|
| **Source Field** | M_RECOVERY_ | M_RECOVERY1 |
| **Propagation** | WITH | WITHOUT |
| **Field Name** | RR01_SENSI | RECOVERY_RATE_SENSI |
...
| Property | Value |
|----------|-------|
| **Request Name** | ER_VESPA_CR_RR01 |
| **Extractor** | DE_VESPA_CR_RR01 |
| **Output Format** | CSV |
...
| 12 | RR01_SENSI | VSP.M_RECOVERY_ | VSP.M_RECOVERY_ |
...
|--------|-------------|-------------|
| **LN** | LN_MR_VESPA_R1_RPT | London RR01 processing |
| **HK** | HK_MR_VESPA_R1_RPT | Hong Kong RR01 processing |
| **NY** | NY_MR_VESPA_R1_RPT | New York RR01 processing |
| **SP** | SP_MR_VESPA_R1_RPT | Singapore RR01 processing |

### 6.2 Batch Event Configuration

| Region | Batch Event | Dependency | Schedule |
|--------|-------------|------------|----------|
| **LN** | BE_VESPA_R1_LN | Valuation complete | 03:00 GMT |
| **HK** | BE_VESPA_R1_HK | Valuation complete | 21:00 HKT |
| **NY** | BE_VESPA_R1_NY | Valuation complete | 22:00 EST |
| **SP** | BE_VESPA_R1_SP | Valuation complete | 21:00 SGT |
...
| CR_RR01 | Feed type identifier | CR_RR01 |
...
| **Function** | Compress and package CR RR01 with other VESPA feeds |
| **Output** | MxMGB_MR_Credit_Sens_{Region}_{YYYYMMDD}.zip |

### 8.2 Files Included in Package

The CR RR01 file is packaged with other CR feeds:
...
| Null RR01_SENSI | Sensitivity must exist | Flag for review |
...
|----------------|--------|--------|-----------|
| Position count | CR Delta Zero | CR RR01 | 0% |
| RR01 values | VW_Vespa_Sensitivities | Feed output | 0.01% |
| RR01 vs RR02 | CR RR01 | CR RR02 | Flag where RR01 < RR02 |
...
| RR01 calculation missing | Valuation failed | Check pricing batch |
